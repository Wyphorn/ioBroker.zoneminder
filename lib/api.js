...
class ZoneMinder extends EventEmitter {
    ...
    _webSocket() {
        ...
        that.client.on('connectFailed', function (error) {
            that.adapter.log.error('WEBSOCKET: Error: client: ' + host + ' Message: ' + error.toString());
    ...
    _get2(path, post, cb) {
        ...
        if ((!that.access_token && !that.authkey && !that.isAuth) && trys === 0) {
            that.adapter.log.error('GET_ ' + !that.access_token + ' ' + !that.access_token + ' ' + !that.isAuth); // API call failed...
        ...
        that.adapter.log.debug('REQUEST: Options ' + that.options.host + path + " params: " + JSON.stringify(options));
        fetch(that.options.host + path, options)
            .then(res => res.json())
            .then(function (json) {
                that.adapter.log.debug('REQUEST: Get data ' + JSON.stringify(json));
                cb(null, json);
            })
            .catch(function (err) {
                that.adapter.log.error(err.message + ' '+ err.name + ' ' + err.stack);
                that.emit('error', err);
    ...
    _getVideo(path, vidId, cb) {
        ...
        fetch(that.options.host + path, options).pipe(Fs.createWriteStream('/opt/iobroker/test_ada_vid.mp4'))
    ...
    auth(callback) {
        ...
        that._get2(auth_url, that.refresh_token ? '' : loginJSON, (e, json) => {
            if (e) {
                that.isAuth = false;
                return;
            }
            ...
            if (that.authkey !== null || that.access_token !== null) {
    ...
    _monitors(all, callback) {
        ...
        this._get2('/api/monitors.json?&' + that.authUrl, (e, json) => {
            ...
            if (!devices.length) {
                if (typeof callback === 'function') return callback(null, json);
            }
